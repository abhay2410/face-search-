...
ort.set_default_logger_severity(3) # Silence ONNX Runtime globally
...
def _make_analyzer(det_size: tuple, det_thresh: float) -> FaceAnalysis:
    ...
    # We set these to ensure that internal sessions stay quiet
...
def _get_analyzer(enrol_mode: bool = False) -> FaceAnalysis:
    ...
    if _analyzer is None:
        ...
        # Using Monitor size (480x480) for monitoring to save performance
        _analyzer = _make_analyzer(config.ARC_FACE_DET_SIZE_MONITOR, det_thresh=config.DET_THRESHOLD)
...
async def load_index():
    ...
    log.info("[Engine] Fetched data for %d patients.", len(all_emps))
...
def _add_to_index_sync(patient_id: int, embeddings: Union[np.ndarray, List[np.ndarray]]):
    global _index, _index_ids
...
def search_index_multi(embeddings: np.ndarray) -> List[Tuple[Optional[int], float]]:
    ...
    for q in range(len(embeddings)):
        ...
        is_highly_confident = top_score > 0.70
        if top_score < config.FAISS_COSINE_THRESHOLD or (
            not is_highly_confident and 
            (top_score - second_score < _MIN_SCORE_GAP and second_score > 0)
        ): 
...
async def extract_faces_full(image: Union[bytes, np.ndarray], enrol_mode: bool = False) -> List[Dict]:
    ...
    # Serialised GPU access
    async with GPU_LOCK:
        # InsightFace internally resizes for detection based on det_size,
        # but uses the full resolution for alignment/recognition.
        faces = await asyncio.get_event_loop().run_in_executor(None, analyzer.get, img)
    
    results = []
    for f in faces:
        if f.normed_embedding is None: continue
        results.append({
            "face": f,
